...
export default function GachaScreen({ onBack }: GachaScreenProps) {
  const { diamonds, spendDiamonds, addCharacter } = useGameStore()
  ...
  const doGacha = (count: number) => {
    ...
    // 引いたキャラを所持キャラに追加
    pulled.forEach(char => {
      const maxLevel = char.rarity === 'SSR' ? 60 : char.rarity === 'SR' ? 50 : 40
      const hp = char.rarity === 'SSR' ? 900 : char.rarity === 'SR' ? 750 : 650
      const attack = char.rarity === 'SSR' ? 80 : char.rarity === 'SR' ? 60 : 45
      addCharacter({
  ...
  useEffect(() => {
    ...
    if (phase === 'scan_burst') {
      const timer = setTimeout(() => {
        if (pullCount === 1) { setCurrentReveal(0); setPhase('reveal') }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        alignItems: 'center', padding: '12px 20px', borderBottom: '1px solid #222244', zIndex: 10,
      }}>
        <div onClick={phase === 'menu' ? onBack : resetGacha} style={{ color: '#666688', fontSize: 14, cursor: 'pointer' }}>
          ← {phase === 'menu' ? '戻る' : 'ガチャトップ'}
        </div>
        <div style={{ color: '#ffffff', fontSize: 18, fontWeight: 'bold' }}>🎲 ガチャ</div>
        <div style={{ color: '#00ccff', fontSize: 14 }}>💎 {diamonds.toLocaleString()}</div>
      </div>

      {phase === 'menu' && (
        <div style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
          <div style={{
            background: 'linear-gradient(135deg, #1a0a2a, #2a1a3a, #0a0a2a)',
            border: '1px solid #aa44ff', borderRadius: 12, padding: '25px 40px', textAlign: 'center', maxWidth: 400,
          }}>
            ...
            <div style={{ marginTop: 15, display: 'flex', justifyContent: 'center', gap: 10 }}>
              <span style={{ fontSize: 30 }}>👑</span><span style={{ fontSize: 30 }}>👁️</span><span style={{ fontSize: 30 }}>🎵</span>
            </div>
            <div style={{ color: '#ffcc00', fontSize: 11, marginTop: 5 }}>ピックアップ: 鷹宮 零 / 明石 瞬 / 響 彩音</div>
          </div>
          <div style={{ display: 'flex', gap: 20 }}>
            <button onClick={() => doGacha(1)} style={{
              background: 'linear-gradient(135deg, #4444aa, #6644cc)', color: '#fff',
              border: 'none', borderRadius: 10, padding: '15px 30px', cursor: 'pointer', fontSize: 16, fontWeight: 'bold',
            }}>単発ガチャ<div style={{ fontSize: 11, color: '#ccccff', marginTop: 3 }}>💎 50</div></button>
            <button onClick={() => doGacha(10)} style={{
              background: 'linear-gradient(135deg, #cc4400, #ff6622)', color: '#fff',
              border: 'none', borderRadius: 10, padding: '15px 30px', cursor: 'pointer', fontSize: 16, fontWeight: 'bold',
            }}>10連ガチャ<div style={{ fontSize: 11, color: '#ffccaa', marginTop: 3 }}>💎 450（お得！）</div></button>
          </div>
        </div>
      )}

      {(phase === 'scan_start' || phase === 'scan_wave' || phase === 'scan_gauge' || phase === 'scan_burst') && (
        <div style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          <div style={{ position: 'absolute', width: '100%', height: '100%', background: phase === 'scan_burst' ? `radial-gradient(circle, ${scanColor}33 0%, transparent 60%)` : 'transparent', transition: 'background 0.5s' }} />

          {phase === 'scan_start' && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: '#00ccff', fontSize: 14, letterSpacing: 8 }}>SCANNING</div>
              <div style={{ color: '#ffffff', fontSize: 28, fontWeight: 'bold', marginTop: 10 }}>第六感覚野を解析中...</div>
            </div>
          )}

          {phase === 'scan_wave' && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: scanColor, fontSize: 14, letterSpacing: 4, marginBottom: 20 }}>NEURAL WAVE DETECTED</div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, height: 60 }}>
                {Array.from({ length: 30 }).map((_, i) => (
                  <div key={i} style={{ width: 4, borderRadius: 2, background: scanColor, height: 30, animation: `wave 0.8s ease-in-out ${i * 0.05}s infinite alternate` }} />
                ))}
              </div>
          ...
          {phase === 'scan_gauge' && (
            <div style={{ textAlign: 'center', width: '80%', maxWidth: 400 }}>
              <div style={{ color: scanColor, fontSize: 14, letterSpacing: 4, marginBottom: 15 }}>AWAKENING LEVEL</div>
              <div style={{ background: '#0a0a1a', borderRadius: 10, height: 30, border: `2px solid ${scanColor}44`, overflow: 'hidden', position: 'relative' }}>
                <div style={{ width: `${gaugeLevel}%`, height: '100%', borderRadius: 8, background: `linear-gradient(90deg, #4466aa, ${scanColor})`, transition: 'width 0.1s', boxShadow: `0 0 20px ${scanColor}66` }} />
                <div style={{ position: 'absolute', left: '35%', top: 0, bottom: 0, borderLeft: '2px dashed #ffffff33' }} />
                <div style={{ position: 'absolute', left: '65%', top: 0, bottom: 0, borderLeft: '2px dashed #ffffff33' }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, padding: '0 5px' }}>
              ...
              <div style={{ color: scanColor, fontSize: 40, fontWeight: 'bold', marginTop: 20 }}>{gaugeLevel}%</div>
            </div>
          )}

          {phase === 'scan_burst' && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 60, fontWeight: 'bold', color: scanColor, textShadow: `0 0 30px ${scanColor}, 0 0 60px ${scanColor}`, animation: 'pulse 0.5s ease infinite' }}>
                {gaugeLevel >= 95 ? 'SSR' : gaugeLevel >= 60 ? 'SR' : 'R'}
              </div>
              <div style={{ color: '#ffffff', fontSize: 22, fontWeight: 'bold', marginTop: 15 }}>覚醒完了！</div>
            </div>
          )}

          <div onClick={skipAnimation} style={{
            position: 'absolute', bottom: 30, right: 20, color: '#666688', fontSize: 13, cursor: 'pointer',
            background: '#111133', borderRadius: 6, padding: '8px 16px', border: '1px solid #333355',
          }}>スキップ ▶▶</div>
        </div>
      )}

      {phase === 'reveal' && results.length > 0 && (
        <div onClick={nextReveal} style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
          {(() => {
            ...
            return (
              <div style={{ textAlign: 'center' }}>
                <div style={{
                  ...
                  border: `3px solid ${config.color}`, boxShadow: `0 0 30px rgba(${config.glowColor}, 0.3)`,
                  ...
                  <div style={{ color: config.color, fontSize: 20, fontWeight: 'bold', marginTop: 10 }}>{config.label}</div>
      ...
      {phase === 'results' && (
        <div style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '10px' }}>
          <div style={{ color: '#ffffff', fontSize: 18, fontWeight: 'bold', marginBottom: 15 }}>ガチャ結果</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10, maxWidth: 500 }}>
            {results.map((char, i) => {
              const config = rarityConfig[char.rarity]
              return (
                <div key={i} style={{
                  width: results.length === 1 ? 150 : 85, height: results.length === 1 ? 190 : 115, borderRadius: 10,
                  background: `linear-gradient(135deg, ${config.bg}, #0a0a15)`,
                  border: `2px solid ${config.color}`, boxShadow: `0 0 15px rgba(${config.glowColor}, 0.2)`,
                  display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                }}>
                  <span style={{ fontSize: results.length === 1 ? 40 : 24 }}>{char.icon}</span>
                  <div style={{ color: config.color, fontSize: results.length === 1 ? 14 : 9, fontWeight: 'bold', marginTop: 4 }}>{config.label}</div>
                  <div style={{ color: '#fff', fontSize: results.length === 1 ? 14 : 10, fontWeight: 'bold', marginTop: 2, textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 20, display: 'flex', gap: 15 }}>
            <button onClick={resetGacha} style={{
              ...
              border: 'none', borderRadius: 8, padding: '10px 30px', fontSize: 14, fontWeight: 'bold', cursor: 'pointer',
            }}>もう一度引く</button>
